...
class Topics{
    ...
    public function addToDatabase(){
        $conn = Db::getInstance();
        $stmt = $conn->prepare("INSERT INTO Users_Topics(id, topics_id) SELECT u.id, t.id from Users u, Topics t where (u.username = :username and t.description = :description)");
    ...
    public function checkTopics(){
        ...
        while ($id = $userID->fetch()){
            $top = $conn->prepare("select * from Users_Topics where user_ID = :userID");
            $top->bindValue(":userID", $id['Name']);
            $top->execute();
            return $top->rowCount();
        }
